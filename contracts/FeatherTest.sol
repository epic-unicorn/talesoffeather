...
contract FeatherTest is ERC721, Ownable, ERC721Enumerable {
    ...
    string public baseURI = ""; // NFT base URI will be set after reveal & sold out
    ...
    uint256 public maxTeamReserve = 64; // Max amount of tokens which can be reserved by team

    event FeatherMinted(uint256 tokenId, address owner);

    constructor() ERC721("FeatherTest", "FET") {}
    ...
    {
        require(
            _reserveAmount > 0 && _reserveAmount <= maxTeamReserve,
        ...
        maxTeamReserve = maxTeamReserve.sub(_reserveAmount);
    }
    ...
    function mintFeather(uint256 numberOfTokens) external payable {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            ...
            if (totalSupply() < maxSupply) {
                ...
                emit FeatherMinted(id, msg.sender);
            }
        }
    }

    function presaleFeather(uint256 numberOfTokens) external payable {
